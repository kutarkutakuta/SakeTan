alter table public.shops
  ...
  add constraint shops_google_place_id_not_blank check (
    google_place_id is null or length(trim(google_place_id)) > 0
  ),
  add constraint shops_manual_search_fields_required check (
    source is not null
    or (
      nullif(trim(name_kana), '') is not null
      and latitude is not null
      and longitude is not null
    )
  ) not valid;

create unique index shops_google_place_id_unique
  on public.shops(google_place_id)
  where google_place_id is not null;

alter table public.shops drop constraint shops_geocode_metadata;
...
language sql stable security invoker set search_path='' as $$
  ...
  where s.is_active
    ...
    and (
      ...
      or position(lower(p_query) in lower(coalesce(s.address, ''))) > 0
    )
    and (
      p_brand_id is null
      or exists (
