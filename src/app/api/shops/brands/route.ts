...
type ShopBrandRow = ShopBrandSummary & { shop_id: string };
...
export async function GET(request: Request) {
  ...
  const { data, error } = await db
    .from("shop_brands")
    .select(
      "shop_id,last_seen_at,brands!inner(id,name,name_kana,brewery_id,sakenowa_rank,sakenowa_score,sakenowa_rank_year_month)",
    )
  ...
  for (const row of (data ?? []) as unknown as ShopBrandRow[]) {
